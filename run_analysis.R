# This R script uses human activity data built from recordings of users of smartphones. 
...
download.data = function(){
  # Checks for data directory, creates it if it doesn't exist
  if (!file.exists("data")) {
...
extract.mean.and.std = function(df) {
  # Given the dataset (x values), extract measurements on the mean and sd.
...
clean.data = function() {
  # Download data
  download.data()
  # Merge datasets and return list of three dataframes: X, y, and subject
  merged <- merge.datasets()
  # Extract the measurements of the mean & sd for each measurement
